...
describe("Master Yield Adapters", function () {
  ...
  describe("CurveMasterAdapter", function () {
    let curvePoolMock: any;
    let curveAdapter: any;

    beforeEach(async function () {
      const MockCurvePool = await ethers.getContractFactory("MockCurvePool");
      curvePoolMock = await MockCurvePool.deploy();

      const CurveMasterAdapter = await ethers.getContractFactory("CurveMasterAdapter");
      curveAdapter = await CurveMasterAdapter.deploy(usdcMock.target, curvePoolMock.target);
    });

    it("should deploy as an ERC20 receipt token with 6 decimals", async function () {
      expect(await curveAdapter.symbol()).to.equal("ccUSDC");
      expect(await curveAdapter.decimals()).to.equal(6);
    });

    it("should supply USDC to Curve pool and mint ccUSDC receipt tokens", async function () {
      const depositAmount = ethers.parseUnits("100", 6);

      // Approve adapter to take USDC
      await usdcMock.connect(user).approve(curveAdapter.target, depositAmount);

      // Call supply
      await expect(curveAdapter.connect(user).supply(usdcMock.target, depositAmount))
        .to.emit(curveAdapter, "Transfer") // ERC20 mint event
        .withArgs(ethers.ZeroAddress, user.address, depositAmount);

      // Check user received ccUSDC receipt tokens
    ...
    it("should revert if supplying an unsupported asset", async function () {
      ...
      await expect(curveAdapter.connect(user).supply(wethMock.target, depositAmount))
